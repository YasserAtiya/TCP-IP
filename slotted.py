...
inp = open('traffic', 'r+')
output = open('slotted_output.txt', 'w+')
...
while (i < total_packets):
   ...
   window = mess.time+mess.pkt_size
   
   #Grab following Message
   if(i < total_packets-1 and grab):
   ...
   #Detect if collision
   if nmess.time < window:

      q.put(nmess)

      #Handle extra layer of security
      nmess2 = Next_Message(inp)
      if(nmess2.time < window):
         nmess.time = mess.time+mess.pkt_size
